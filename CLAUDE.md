...
- Use prefixes: `feat`, `fix`, `docs`, `chore`, `ci`, `build`, `refactor`, `perf`, `test`, `style`, `revert`.
...
- **Merge**: Verify release label (`release:{major,minor,patch}`), merge, delete remote branch, pull main,
delete local branch.
...
- **CONSIDER**: Evaluate once. Implement if valuable, else reject with short reasoning.
Ignore repeats on unchanged code.
- **Completion**: Done when all MUST FIX resolved and comments answered.
Do not cycle on repeated CONSIDERs; open issues to defer.
